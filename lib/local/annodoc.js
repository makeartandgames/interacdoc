...
//         		      function(a, b) { return b.replace(/\s+/g, ' '); });
    ...
    var parseSd = function(sd, log) {
        ...
        function findNthIndex(arr, element, n) {
            var count = 0;
        ...
        var tokenIndex = function(t) {
            ...
            if (m) {
                ...
                // TODO: TODOMIKE TODO-MIKE
                 idx += 1;
                idx =  findNthIndex(tokens, text, idx);
                console.log('text', text)
                // END MIKE's contribution 
                if (tokens[idx] === text) {
                    return idx;
                } else {
        ...
        // TODO TODOmike TODO-MIKE://
        /////////////////////////////
        // reinitializing span via filter is Mike's addition
        // it allows you to use word/POS syntax selectively. 
        // This code ensures that words without POS labels will get empty labels if they are in relations.
        // before this change, word/POS would highlight every word even if you didn't give it a /POS.
        // 'highlighting' is determined by presnece in the 'spans' array (which is BRAT parse)
        // with a T<index> label. Relations are dependent on these labels. 
        // this code removes a span if it is an empty POS and doesn't have a relation.
        // it works.
        spans = spans.filter( ([ts, xPOS, _2]) => {
     // spans.push(['T'+(i+1), POS, [[offset, offset+length]]]); <<== this line is here for reference
            var save = null;
            var inRelation = relations.some( ([_a, _b, [ [_c, t1], [_d, t2]] ]) => {
                save = ts;
                return  (ts === t1) || (ts === t2);
            })
            var hasPOS = xPOS !== 'token'; // for some reason empty POS are assigned '.'
            if (ts === 'T10') console.log(save, hasPOS, xPOS, inRelation)
            return hasPOS || inRelation;
        }
        );
    ...
    var displayArcInformation = function(evt, target, symmetric, arcId,
        ...
        var comment = "";
        // TODO: check below but looks disabled.
